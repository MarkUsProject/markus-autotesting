...
remove_unprivileged_user() {
	local workeruser=$1
    read -p "Are you sure you want to remove the user: ${workeruser} " -n 1 -r
...
remove_server_user() {
    if [[ -z ${SERVERUSER} ]]; then 
	    ...
	    read -p "Are you sure you want to remove the server user: ${SERVERUSER} " -n 1 -r
		echo    # (optional) move to a new line
		if [[ $REPLY =~ ^[Yy]$ ]]; then
...
remove_working_directory() {
	...
	read -p "Do you want to continue removing the working directory without archiving it first?" -n 1 -r
	echo    # (optional) move to a new line
	if [[ $REPLY =~ ^[Yy]$ ]]; then
...
uninstall_testers() {
	for installed in $(ls "${TESTERSDIR}/*/specs/.installed"); do
		...
		read -p "Do you want to uninstall the installed tester: ${tester}" -n 1 -r
		echo    # (optional) move to a new line
		if [[ $REPLY =~ ^[Yy]$ ]]; then
		    ${uninstall_script}
		fi
