...
create_data_set_str() {
    ...
    " | cat - ${schemafile} ${datafilepath} <("GRANT SELECT ON ALL TABLES IN SCHEMA ${schemaname} TO ${alltestusers};")
}
	...
	for testname in "${method_names[@]}"; do
    	...
    	for datafile in "${datasets[@]}"; do
    		...
    		if [[ "${schemas}" != *" ${schemaname} "* ]]; then # first time using this dataset, create a schema for it
    			psql -U ${oracleuser} -d ${oracledb} -h localhost -f <(create_data_set_str schemaname datafile)
    			schemas="${schemas} ${schemaname} "
    		fi
    		psql -U ${oracleuser} -d ${oracledb} -h localhost -f <(create_solution_str schemaname testnamedb)
